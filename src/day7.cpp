...
connection *
getConnection(memory_arena *Arena,
              char *Line)
{
  char *scratch, *txt;

  const int32 DATA_LENGTH = 5;
  char *Data[DATA_LENGTH];
  int32 NumStrings = SplitString(Line, Data, DATA_LENGTH, ' ');
  ...
  {
    NewCon->Op = OP_DEFINE;
    SafeStrCpy(NewCon->Variable, Data[2], DATA_LENGTH);
    SafeStrCpy(NewCon->Operands[0], Data[0], VALUE_LEN);
    NewCon->NumOperands = 1;
  }
  // NOT x -> y
  else if (NumStrings == 4)
  {
    NewCon->Op = OP_NOT;
    SafeStrCpy(NewCon->Variable, Data[3], DATA_LENGTH);
    SafeStrCpy(NewCon->Operands[0], Data[1], VALUE_LEN);
    NewCon->NumOperands = 1;
  }
  // x OP y -> z
  else if (NumStrings == 5)
  {
    NewCon->Op = getOperation(Data[1]);
    SafeStrCpy(NewCon->Variable, Data[4], DATA_LENGTH);
    SafeStrCpy(NewCon->Operands[0], Data[0], VALUE_LEN);
    SafeStrCpy(NewCon->Operands[1], Data[2], VALUE_LEN);
    NewCon->NumOperands = 2;
  }
...
void
insertEquation(memory_arena *Arena,
               equation **ENode,
               equation *Equation)
{
  if(!(*ENode))
...
void
BuildEquationTree(memory_arena *Arena,
                  filelines *Day7,
...
char *
CalculateTreeNode(equation *ENode,
                  equation **EquationTree)
{
  char *Value = 0;
  if(ENode->isOperated)
